...
public class reviewerRequestPage extends VBox {
    ...
    public reviewerRequestPage(User admin, DatabaseHelper databaseHelper) {
    ...
    public void show(Stage primaryStage) {
        ...
        //header
        Label headerLabel = new Label("Reviewer Role Requests");
...
//        userTable.getColumns().add(actionCol);
        ...
        backButton.setOnAction(e -> new InstructorHomePage(databaseHelper, currentAdmin).show(primaryStage));
        ...
        buttonBar.getChildren().addAll(refreshButton, backButton, acceptButton, DenyButton);
    ...
    private void handleAddRole(User user, String role) {
        try {
            if (databaseHelper.addRole(user.getUserName(), role)) {
                showAlert("Success", "Request Accepted", Alert.AlertType.INFORMATION);
                refreshUserList();
            } else {
...
//    }
    ...
    private ArrayList<User> getRequestedReviewers() {
        ...
        for (User user : users) {
            ...
            if (role.contains("requestedreviewer")) {
